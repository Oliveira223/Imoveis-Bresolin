...
// ========================================
// MÓDULO: GERENCIAMENTO DE INTERESSE
// ========================================

/**
 * Controla a seção de cadastros de interesse dos clientes
 */
const InteresseModule = {
  /**
   * Carrega e exibe todos os cadastros de interesse
   */
  async carregarInteresses() {
    const lista = document.getElementById('lista-interesse');
    if (!lista) return;
    
    lista.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 2em;">Carregando...</td></tr>';
    ...
    try {
      ...
      if (interesses.length === 0) {
         lista.innerHTML = '<tr><td colspan="5" class="interesse-empty">Nenhum cadastro de interesse encontrado</td></tr>';
         return;
       }
      
      lista.innerHTML = '';
      interesses.forEach(interesse => {
        lista.appendChild(this.criarLinhaInteresse(interesse));
      });
      
    } catch (error) {
      console.error('Erro ao carregar interesses:', error);
      lista.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2em; color: #e74c3c;">Erro ao carregar dados</td></tr>';
    }
  },
  
  /**
   * Cria uma linha da tabela para um cadastro de interesse
   */
  criarLinhaInteresse(interesse) {
    const tr = document.createElement('tr');
    
    // Formatar data
    const data = new Date(interesse.timestamp);
    const dataFormatada = data.toLocaleDateString('pt-BR') + ' às ' + data.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
    
    // Formatar WhatsApp (remover caracteres especiais para o link)
    const whatsappLimpo = interesse.whatsapp.replace(/\D/g, '');
    const whatsappFormatado = interesse.whatsapp;
    
    tr.innerHTML = `
       <td><strong>${this.escapeHtml(interesse.nome)}</strong></td>
       <td>
         <div class="email-container">
           <span>${this.escapeHtml(interesse.email)}</span>
           <button class="btn-copy-email" onclick="InteresseModule.copiarEmail('${interesse.email}', this)" title="Copiar email">
       ...
       <td>
         <a href="https://wa.me/55${whatsappLimpo}?text=Olá%20${encodeURIComponent(interesse.nome)}!%20Vi%20que%20você%20se%20cadastrou%20no%20nosso%20site%20demonstrando%20interesse%20em%20nossos%20imóveis.%20Como%20posso%20te%20ajudar?" 
            target="_blank" 
            class="whatsapp-link"
       ...
       <td class="interesse-timestamp">${dataFormatada}</td>
       <td class="interesse-acoes">
         <button class="btn-acao ${interesse.atendido ? 'btn-atendido' : 'btn-pendente'}" 
                 onclick="InteresseModule.toggleAtendido(${interesse.id}, ${!interesse.atendido}, this)" 
                 title="${interesse.atendido ? 'Marcar como pendente' : 'Marcar como atendido'}">
           ${interesse.atendido ? '✓ Atendido' : '⏳ Pendente'}
         </button>
         <button class="btn-acao btn-excluir" 
                 onclick="InteresseModule.excluirInteresse(${interesse.id})" 
  ...
  /**
   * Preenche os KPIs da seção de interesse
   */
  async preencherKpisInteresse() {
    try {
      const response = await fetch('/api/interesse');
      ...
      // Total de cadastros
      const totalElement = document.getElementById('kpi-total-interesse');
      if (totalElement) {
        totalElement.textContent = interesses.length;
      }
      ...
      const cadastrosHoje = interesses.filter(interesse => {
        const dataInteresse = new Date(interesse.timestamp);
        dataInteresse.setHours(0, 0, 0, 0);
        return dataInteresse.getTime() === hoje.getTime();
      }).length;
      
      const hojeElement = document.getElementById('kpi-hoje-interesse');
      if (hojeElement) {
        hojeElement.textContent = cadastrosHoje;
      }
      
    } catch (error) {
      console.error('Erro ao carregar KPIs de interesse:', error);
    }
  },
  
  /**
   * Configura os event listeners da seção
   */
  init() {
    const btnAtualizar = document.getElementById('btn-atualizar-interesse');
    if (btnAtualizar) {
      btnAtualizar.addEventListener('click', () => {
        this.carregarInteresses();
        this.preencherKpisInteresse();
      });
    }
  },
  ...
  /**
    ...
    */
   async copiarEmail(email, button) {
     try {
   ...
   /**
    * Marca/desmarca interesse como atendido
    */
   async toggleAtendido(interesseId, atendido, button) {
     try {
       const response = await fetch(`/api/interesse/${interesseId}/atendido`, {
         method: 'PUT',
         headers: {
           'Content-Type': 'application/json'
         },
       ...
       // Atualizar o botão
       button.className = `btn-acao ${atendido ? 'btn-atendido' : 'btn-pendente'}`;
       button.textContent = atendido ? '✓ Atendido' : '⏳ Pendente';
       button.title = atendido ? 'Marcar como pendente' : 'Marcar como atendido';
       button.onclick = () => this.toggleAtendido(interesseId, !atendido, button);
       
       // Mostrar feedback
       const status = atendido ? 'atendido' : 'pendente';
       this.mostrarFeedback(`Interesse marcado como ${status}!`, 'sucesso');
       
     } catch (error) {
       console.error('Erro ao atualizar status:', error);
       this.mostrarFeedback('Erro ao atualizar status do interesse', 'erro');
     }
   },
    ...
    * Exclui um interesse
    */
   async excluirInteresse(interesseId) {
     if (!confirm('Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.')) {
       return;
     }
     
     try {
       const response = await fetch(`/api/interesse/${interesseId}`, {
         method: 'DELETE'
       });
       
       if (!response.ok) {
         throw new Error('Erro ao excluir interesse');
       }
       ...
       this.mostrarFeedback('Interesse excluído com sucesso!', 'sucesso');
       
     } catch (error) {
       console.error('Erro ao excluir interesse:', error);
       this.mostrarFeedback('Erro ao excluir interesse', 'erro');
     }
   },
