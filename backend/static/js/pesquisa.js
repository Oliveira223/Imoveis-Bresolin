document.addEventListener('DOMContentLoaded', () => {
  ...
  function aplicarFiltrosCompletos() {
    ...
    // Verificar se algum filtro específico de imóvel está ativo
    ...
    cards.forEach(card => {
      ...
      // Se filtros específicos de imóveis estão ativos, esconder todos os empreendimentos
      if (isEmpreendimento && filtrosEspecificosImoveis) {
        isVisible = false;
      }
      ...
      if (isVisible && isEmpreendimento) {
        // Para empreendimentos, apenas aplicar filtros de área se especificados
        if (filtros.areaMin || filtros.areaMax) {
          const areaElement = card.querySelector('.info-item img[alt="Área"]');
          if (areaElement) {
            const areaTexto = areaElement.parentElement.querySelector('span')?.textContent || '';
            
            // Verificar se é um range (ex: "50 - 120 m²")
            if (areaTexto.includes('-')) {
              const partes = areaTexto.split('-');
              const areaMinEmpreendimento = parseFloat(partes[0].replace(/[^\d.,]/g, '').replace(',', '.'));
              const areaMaxEmpreendimento = parseFloat(partes[1].replace(/[^\d.,]/g, '').replace(',', '.'));
              
              // Se o filtro mínimo é maior que o máximo do empreendimento
              if (filtros.areaMin && areaMaxEmpreendimento < filtros.areaMin) {
                isVisible = false;
              }
              // Se o filtro máximo é menor que o mínimo do empreendimento
              if (filtros.areaMax && areaMinEmpreendimento > filtros.areaMax) {
                isVisible = false;
              }
            } else {
              // Área única ou com "+" (ex: "100+ m²")
              const areaNumero = parseFloat(areaTexto.replace(/[^\d.,]/g, '').replace(',', '.'));
              
              if (areaTexto.includes('+')) {
                // Para "100+ m²", só verifica o máximo
                if (filtros.areaMax && areaNumero > filtros.areaMax) {
                  isVisible = false;
                }
              } else {
                // Área única
                if (filtros.areaMin && areaNumero < filtros.areaMin) {
                  isVisible = false;
                }
                if (filtros.areaMax && areaNumero > filtros.areaMax) {
                  isVisible = false;
        ...
        // Para empreendimentos, ocultar apenas se filtros específicos de imóveis estão ativos
        // (mas permitir que apareçam se apenas área está filtrada)
        if (filtros.quartos !== null || filtros.banheiros !== null || filtros.vagas !== null || filtros.piscina || filtros.churrasqueira) {
          // Não ocultar automaticamente - deixar que apareçam nos resultados
          // isVisible = false;
        }
      }
  ...
  // ================================
  // Preço editável formatado (aplicar filtros)
  // ================================
  if (precoEditavel) {
    precoEditavel.addEventListener('input', function () {
      let texto = this.innerText.replace(',00', '').replace(/\D/g, '');
